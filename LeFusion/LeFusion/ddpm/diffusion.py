...
class GaussianDiffusion_Nolatent(nn.Module):
    ...
    def p_losses(self, x_start, t, mask, cond=None, noise=None, **kwargs):
        ...
        if self.data_type == 'lidc':
            ...
            # For FCD2, we have binary masks (0=background, 1=FCD)
            # Convert mask to float for multiplication
            mask_float = (mask == 1).float()
            
            # Apply mask to focus on FCD regions (exactly like LIDC)
            noise = noise * mask_float
            x_recon = x_recon * mask_float
            
            # Calculate loss on masked regions
            if self.loss_type == 'l1':
                loss = F.l1_loss(noise, x_recon)
            elif self.loss_type == 'l2':
                loss = F.mse_loss(noise, x_recon)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.ds = dataset
        dl = SubjectsLoader(self.ds, batch_size=train_batch_size, shuffle=True, num_workers=num_workers)
